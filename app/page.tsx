...
import { BonsaiIcon, HomeIcon, CoinIcon, SeedlingIcon, TreeIcon } from "@/components/BonsaiIcon";
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section
        ...
        style={{ background: "linear-gradient(160deg,#e8f5e9 0%,#f5f0e8 50%,#fff8e7 100%)" }}>
        <div className="mb-4" style={{ filter: "drop-shadow(0 4px 8px rgba(0,0,0,0.15))" }}>
          <TreeIcon className="w-20 h-20 mx-auto" />
        </div>
        ...
        <Link
          href="/game"
          className="bg-green-800 text-white text-lg font-bold px-8 py-4 rounded-2xl shadow-lg hover:bg-green-700 active:scale-95 transition-all min-h-[56px] flex items-center gap-2"
          aria-label="ゲームを始める">
          ...
          {[
            {
              ...
              title: "起動するたびに報酬",
              desc: "アプリを閉じている間も盆栽は育ち続ける。次に開いたとき、コインが溜まっている！"
            },
            {
              icon: <SeedlingIcon className="w-8 h-8" />,
              title: "育てて売る爽快感",
              desc: "松→梅→桜→紅葉→藤。希少な盆栽ほど高値で売れる。庭を拡張して収益を最大化！"
            },
            {
              icon: <BonsaiIcon icon="water" className="w-8 h-8" aria-label="水やりアイコン" />,
              title: "水やり・肥料で加速",
              desc: "水やりや肥料で成長スピードがアップ。剪定ばさみを使えば売値も20%UP！"
            },
            {
              icon: <HomeIcon className="w-8 h-8" />,
              title: "庭を広げて大農場",
              desc: "最初は4スロット。コインを貯めて庭を拡張。いつかは大きな盆栽農園に！"
            },
          ].map((f, i) => (
          ...
          {BONSAI_TYPES.map(b => (
            <div key={b.id} className="bg-white rounded-2xl p-4 border border-amber-100 text-center w-24 shadow-sm">
              <div className="flex justify-center mb-1">
                <BonsaiIcon icon={b.icon} className="w-10 h-10" aria-label={b.name} />
              </div>
      ...
      <section className="px-6 py-12 text-center bg-green-800">
        ...
        <Link
          href="/game"
          className="bg-yellow-400 text-yellow-900 text-lg font-bold px-8 py-4 rounded-2xl shadow-lg hover:bg-yellow-300 active:scale-95 transition-all inline-flex items-center gap-2 min-h-[56px]"
          aria-label="庭を始める">
          <HomeIcon className="w-6 h-6" />
          庭を始める
        </Link>
