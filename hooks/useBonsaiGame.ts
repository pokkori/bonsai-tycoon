...
export function useBonsaiGame(){
  ...
  const plantBonsai=useCallback((plotIndex:number,typeId:string)=>{
    setState((prev)=>{
      ...
      const newInstance:BonsaiInstance={id:typeId+"_"+Date.now(),plotIndex,typeId,level:1,plantedAt:Date.now(),waterings:0,fertilizers:0,state:"growing",growthProgress:0};
  ...
  const harvestBonsai=useCallback((plotIndex:number)=>{
    setState((prev)=>{
      ...
      const msg=useScissors?"✂️ 剪定済み！ "+earned.toLocaleString()+"コイン獲得！":"🌟 収穫！ "+earned.toLocaleString()+"コイン獲得！";
      showNotification(msg);
      addFloatCoin(earned);
      return{...prev,coins:prev.coins+earned,totalEarned:prev.totalEarned+earned,totalHarvested:prev.totalHarvested+1,items:useScissors?{...prev.items,scissors:prev.items.scissors-1}:prev.items,plots:newPlots};
    });
  ...
  const buyItem=useCallback((itemType:"water"|"fertilizer"|"scissors")=>{
    setState((prev)=>{
      ...
      showNotification(item.emoji+" "+item.name+"を購入しました");
  ...
  const unlockBonsaiType=useCallback((typeId:string)=>{
    setState((prev)=>{
      ...
      showNotification(type.emoji+" "+type.name+"の盆栽を解放しました！");
